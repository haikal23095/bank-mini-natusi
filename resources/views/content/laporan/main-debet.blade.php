                        ...
                        <table id="datatabel" class="table table-striped table-bordered" width="100%">
                            <thead>
                                <tr>
                                    ...
                                    <td class="text-center">JUMLAH DEBET</td>
...
<script>
    ...
    function loadTable(filterBy, filter = '', startDate = '', endDate = ''){
        ...
        table.on('xhr', function() {
            var tDebet = table.ajax.json().tDebet;
            $('#totalDebet').text(formatRupiah(tDebet, 'Rp. '));
        });
    }
    ...
    function searchData() {
        ...
        var filterBy = $('#filterBy').val(); // value select option filter
        if (startDate && endDate) {
            $('#datatabel').DataTable().destroy();
            loadTable(filterBy, filter, startDate, endDate);
        } else if(filterBy && filter && startDate && endDate) {
            $('#datatabel').DataTable().destroy();
            loadTable(filterBy, filter, startDate, endDate);
        } else {
            Swal.fire({
    ...
    function deleteData(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post("{{route('deleteLapDebet')}}",{id:id})
                .done((data) => {
                    if(data.status == "success"){
